...
class PinkPantherEnv(gym.Env):
    ...
    def reset(self):
        ...
        p.setGravity(0,0,-10)
        planeId = p.loadURDF("envs/assets/PinkPanther/plane/plane.urdf")
        robotStartPos = [0,0,0.2]
        robotStartOrientation = p.getQuaternionFromEuler([0,0,0])
        self.robotid = p.loadURDF("envs/assets/PinkPanther/urdf/PinkPanther_CML.urdf", robotStartPos, robotStartOrientation)
        self.p, self.q = p.getBasePositionAndOrientation(self.robotid)
        self.p, self.q = np.array(self.p), np.array(self.q)
        self.i = 0
        return self.get_obs()
